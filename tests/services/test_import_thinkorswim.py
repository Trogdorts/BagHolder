...
def test_compute_daily_pnl_records_tracks_invested_amount():
    records = [
        {
            "date": date(2024, 2, 1),
            "side": "BUY",
    ...
    df = compute_daily_pnl_records(records)
    assert list(df["date"]) == [date(2024, 2, 1), date(2024, 2, 2)]

    first_day = df.iloc[0]
    assert first_day["realized_pl"] == pytest.approx(0.0)
    assert first_day["unrealized_pl"] == pytest.approx(1550.0)

    second_day = df.iloc[1]
    assert second_day["realized_pl"] == pytest.approx(133.33, abs=0.01)
    assert second_day["unrealized_pl"] == pytest.approx(723.33, abs=0.01)


def test_compute_daily_pnl_records_handles_shorts():
    records = [
        {
            "date": date(2024, 3, 5),
            "side": "SELL",
    ...
    df = compute_daily_pnl_records(records)
    assert list(df["date"]) == [date(2024, 3, 5), date(2024, 3, 6)]

    day_one = df.iloc[0]
    assert day_one["realized_pl"] == pytest.approx(0.0)
    assert day_one["unrealized_pl"] == pytest.approx(-500.0)

    day_two = df.iloc[1]
    assert day_two["realized_pl"] == pytest.approx(20.0)
    assert day_two["unrealized_pl"] == pytest.approx(-300.0)
